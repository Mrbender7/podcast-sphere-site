...
const translations: Record<Language, Record<string, string>> = {
  fr: {
    ...
    "guide.permissionsContent": "Radio Sphere demande quelques autorisations pour fonctionner pleinement : les notifications permettent d'afficher les contrôles de lecture sur l'écran verrouillé ; la localisation est nécessaire pour détecter les appareils Chromecast à proximité ; l'accès au stockage permet de sauvegarder vos enregistrements sur votre téléphone. Aucune donnée personnelle n'est collectée ni envoyée.",
  ...
  en: {
    ...
    "guide.permissionsContent": "Radio Sphere requests a few permissions to work fully: notifications allow playback controls on your lock screen; location is needed to detect nearby Chromecast devices; storage access lets you save your recordings to your phone. No personal data is collected or sent.",
  ...
  de: {
    ...
    "guide.permissionsContent": "Radio Sphere benötigt einige Berechtigungen für den vollen Funktionsumfang: Benachrichtigungen ermöglichen Wiedergabesteuerung auf dem Sperrbildschirm; der Standort wird benötigt, um Chromecast-Geräte in der Nähe zu erkennen; Speicherzugriff ermöglicht das Speichern deiner Aufnahmen auf dem Telefon. Es werden keine persönlichen Daten erfasst oder gesendet.",
  ...
  ja: {
    ...
    "guide.permissionsContent": "Radio Sphereはすべての機能を利用するためにいくつかの許可を求めます：通知はロック画面での再生コントロールの表示に必要です。位置情報は近くのChromecastデバイスの検出に必要です。ストレージへのアクセスは録音をスマートフォンに保存するために必要です。個人データの収集や送信は一切行いません。",
