      ...
      <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {CATEGORIES.map(cat => {
              const style = CATEGORY_STYLES[cat] || { bg: "from-gray-700 to-gray-800", color: "#aaa", glow: "rgba(170,170,170,0.4)" };
              const iconPath = CATEGORY_ICONS[cat];
              return (
                <div
                  key={cat}
                  className={`relative rounded-xl p-4 flex flex-col items-center justify-center bg-gradient-to-br ${style.bg} cursor-pointer active:scale-95 transition-all overflow-hidden`}
                  style={{
                    boxShadow: `inset 0 0 15px ${style.glow}, 0 0 20px ${style.glow.replace("0.5", "0.3")}`,
                    borderColor: style.glow.replace("0.5", "0.2"),
                    borderWidth: 1,
                    borderStyle: "solid",
                    minHeight: 100,
                  }}
                ...
                >
                  {iconPath && (
                    <svg
                      ...
                      className="mb-2 animate-neon-pulse pointer-events-none"
                      style={{ fill: style.color, filter: `drop-shadow(0 0 8px ${style.glow})` }}
                    >
                      <path d={iconPath} />
                    </svg>
                  )}
