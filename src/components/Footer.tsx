...
export function Footer() {
  ...
  return (
    <footer className="hidden lg:flex items-center justify-between gap-4 px-6 py-2 border-t border-border bg-secondary/30 backdrop-blur-sm text-[10px] text-muted-foreground">
      <span>© {new Date().getFullYear()} Podcast Sphere — {t("footer.createdBy")}</span>
      <div className="flex items-center gap-4">
        <a href="https://radiosphere.be/privacy-policy-podcastsphere.html" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-primary transition-colors">
          <ShieldCheck className="w-3 h-3" />
          {t("settings.privacyPolicy")}
        </a>
        <a href="https://podcastindex.org/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-primary transition-colors">
